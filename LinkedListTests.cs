...
namespace Test_Assignment_3
{
    public class LinkedListTests
    {
        ...
        {
            this.linkedList.Append("p");
            this.linkedList.Append("t");
            this.linkedList.Clear();

            // My List would be empty

            Assert.True(this.linkedList.IsEmpty());
            Assert.AreEqual(0, this.linkedList.Size());
        }

        //Will My test still work if I add starting into empty list
        [Test]
        public void TestAddtoStartEMMPTYSLL()
        {
            this.linkedList.Insert("p", 0);

            // My list would be only p at the starting .
            Assert.False(this.linkedList.IsEmpty());
            Assert.AreEqual(1, this.linkedList.Size());
            Assert.AreEqual("p", this.linkedList.Retrieve(0));
        }

        //We will now delete only one element from starting
        [Test]
        ...
        //Wanna test the Exceptions? We will see using retrieve 
        //We did a mistake in it which was resolved by ChatGPT3.5 with Prompt:"What's wrong in it"
        [Test]
        public void TestRetrieveNonValidIndex()
        {
            Assert.Throws<IndexOutOfRangeException>(() => this.linkedList.Retrieve(5));
        }

        //Testing to find a node that never existed
        [Test]
        public void TestFindNonExistingNode()
        {
            Assert.False(this.linkedList.Contains("p"));
            Assert.AreEqual(-1, this.linkedList.IndexOf("p"));
            Assert.Throws<IndexOutOfRangeException>(() => this.linkedList.Retrieve(this.linkedList.IndexOf("p")));
        }
